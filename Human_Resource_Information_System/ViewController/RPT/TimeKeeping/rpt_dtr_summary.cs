...
namespace Human_Resource_Information_System
{
    public partial class rpt_dtr_summary : Form
    {
            ...
            try
            {
                if (dgvl_dtr_sum_files.Rows.Count > 1)
                {
                    ...
                    try
                    {
                        ...
                        {
                            File.Delete(sys_dir + "/ViewController/RPT/TimeKeeping/dtr_summary_pdf/" + dtr_filename);
        ...
        private void bgworker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                date_from = DateTime.Parse(pay_period.Rows[0]["date_from"].ToString()).ToShortDateString();
                date_to = DateTime.Parse(pay_period.Rows[0]["date_to"].ToString()).ToShortDateString();
            }
            ...
            t.AddCell(new PdfPCell(new Phrase(new Chunk("Total Late", font))) { HorizontalAlignment = Element.ALIGN_CENTER });

            t.AddCell(new PdfPCell(new Phrase(new Chunk("Total Undertime", font))) { HorizontalAlignment = Element.ALIGN_CENTER });
            t.AddCell(new PdfPCell(new Phrase(new Chunk("Total Overtime", font))) { HorizontalAlignment = Element.ALIGN_CENTER });

            t.AddCell(new PdfPCell(new Phrase(new Chunk("Reg OT", font))) { HorizontalAlignment = Element.ALIGN_CENTER });
            t.AddCell(new PdfPCell(new Phrase(new Chunk("Dayoff OT", font))) { HorizontalAlignment = Element.ALIGN_CENTER });
            t.AddCell(new PdfPCell(new Phrase(new Chunk("Legal Hol. OT", font))) { HorizontalAlignment = Element.ALIGN_CENTER });
            t.AddCell(new PdfPCell(new Phrase(new Chunk("Special Hol. OT", font))) { HorizontalAlignment = Element.ALIGN_CENTER });
